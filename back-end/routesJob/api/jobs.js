...
router.get('/companyName/:id', (req, res) => {
    const id = req.params.id;
    try {
        pool.query(`SELECT company_information.company_name
        FROM company_information
        INNER JOIN jobs_information ON company_information.company_id=jobs_information.company_id
        where company_information.company_id = ${id}`, (err, result) => {
            if (err) {
                console.log(err);
                return res.status(500).send('server error');
            }
